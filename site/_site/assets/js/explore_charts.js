...
export async function initCategoriesChart() {
  // Query expense data — cast date to string in SQL for reliable parsing
  expenseData = await runQuery(`
    SELECT CAST(date AS VARCHAR) as date, category, SUM(value) as value
  ...
  // Query events — cast dates to string in SQL
  eventData = await runQuery(`
    SELECT CAST(date_start AS VARCHAR) as date_start,
           CAST(date_end AS VARCHAR) as date_end,
           event, type
    FROM events_public
  `);
